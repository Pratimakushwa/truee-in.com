...
//             // ⚡ Sirf wahi coupon laao jo Active hain, Date ke andar hain, aur Checkout par allow kiye gaye hain
//             const activeOnly = (data.coupons || []).filter(c => 
//                 c.status === 'Active' && 
//                 new Date(c.startDate) <= now &&
//                 new Date(c.expiryDate) > now &&
//                 c.showOn?.includes('Checkout Page')
//             );
//             setAvailableCoupons(activeOnly);
//         } catch (error) { console.error("Coupons load nahi hue:", error); }
//     };
//     fetchActiveCoupons();
//   }, []);
//   // --- COUPON LOGIC END ---
...
//   // --- COUPON LOGIC START ---
//   // ⚡ Dynamic categorization of coupons based on subtotal
...
//       const finalCode = typeof codeToApply === 'string' ? codeToApply : couponCode;
//       if (!finalCode) return;
      
//       try {
//           const { data } = await axiosInstance.post('/coupons/verify', { code: finalCode, cartTotal: subtotal });
//           setDiscount(data.discountAmount);
//           setCouponCode(finalCode); // Ensure input reflects the applied code
//           setIsCouponApplied(true);
//           setCouponMessage(data.message || 'Coupon applied successfully!');
//           showToast("success", "Coupon Applied Successfully!");
//           setShowCouponsList(false); // Close dropdown on apply
//       } catch (error) {
//           setDiscount(0);
...
//             const { data: orderData } = await axiosInstance.post("/payment/create-order", {
//                 amount: finalTotal, // ⚡ Updated to use finalTotal
//                 paymentMethod: 'cod',
//                 orderDetails: {
//                     email,
//                     items: formattedItems,
...
//         const options = {
//             key: orderData.key_id, 
//             amount: orderData.order.amount, // This will now reflect the discounted amount
//             currency: orderData.order.currency,
//             name: "Truee Luxury", 
//             description: "Premium Purchase",
//             order_id: orderData.order.id,
...
//             className="w-full bg-black text-white py-4 rounded font-bold tracking-widest hover:bg-gray-900 transition-all uppercase text-sm disabled:opacity-50 disabled:cursor-not-allowed shadow-lg active:scale-95"
//           >
//             {paymentProcessing ? "Processing..." : (paymentMethod === 'razorpay' ? `Pay ₹${finalTotal.toLocaleString('en-IN')}` : "Place Order")}
//           </button>
//         </div>
...
//         <h3 className="text-sm font-semibold uppercase tracking-widest text-gray-800 mb-4">Promo Code</h3>
...
//                     className="w-full border border-gray-300 p-3 text-sm focus:outline-none uppercase bg-gray-50" 
//                 />
//                 <button 
//                     onClick={() => applyCoupon(couponCode)} 
//                     className="bg-black text-white px-6 py-3 text-sm font-bold uppercase hover:bg-gray-800 transition-colors"
//                 >
//                     Apply
//                 </button>
//             </div>
//         ) : (
//             <div className="flex justify-between items-center bg-green-50 p-4 border border-green-200 rounded mb-4">
//                 <span className="text-green-700 font-bold text-sm">{couponCode} APPLIED</span>
...
//         {/* 2. Dropdown Section: Sirf eligible coupons hone par hi dikhega */}
//         {!isCouponApplied && eligibleCoupons.length > 0 && (
//             <div className="mt-2">
//                 <button 
//                     onClick={() => setShowCouponsList(!showCouponsList)} 
//                     className="flex items-center justify-between w-full text-sm font-medium text-gray-700 bg-white border border-gray-300 p-3 rounded hover:bg-gray-50 transition-colors"
//                 >
//                     {showCouponsList ? "Hide Available Offers" : `View Available Offers (${eligibleCoupons.length})`}
//                     <ChevronDown size={16} className={`transform transition-transform ${showCouponsList ? "rotate-180" : ""}`} />
//                 </button>
                
//                 {showCouponsList && (
//                     <div className="mt-3 bg-white border border-gray-100 rounded p-3 space-y-3 shadow-inner max-h-60 overflow-y-auto">
//                         {eligibleCoupons.map(c => (
//                             <div key={c._id} className="flex justify-between items-center p-3 border-dashed border border-gray-300 bg-white rounded">
//                                 <div>
//                                     <span className="font-bold text-sm text-black">{c.code}</span>
//                                     <div className="text-xs text-gray-500 mt-1">{c.description || `Save ${c.discountType === 'percentage' ? c.discountValue + '%' : '₹' + c.discountValue}`}</div>
//                                 </div>
//                                 <button onClick={() => applyCoupon(c.code)} className="text-xs font-bold bg-black text-white px-4 py-2 rounded hover:bg-gray-800 transition-colors">APPLY</button>
//                             </div>
//                         ))}
//                     </div>
//                 )}
...
//                   <span className="text-xl font-serif font-bold text-[#333]">Total</span>
//                   <span className="text-2xl font-bold text-black tracking-tight">₹{finalTotal.toLocaleString('en-IN')}</span>
//                 </div>
...
//                     className="w-full bg-black text-white py-5 rounded-md text-sm sm:text-[15px] font-bold shadow-xl active:scale-95 transition-all tracking-widest uppercase disabled:opacity-50 hover:bg-gray-900"
//                   >
//                     {paymentProcessing ? "Processing..." : (paymentMethod === 'razorpay' ? `Checkout with Razorpay (₹${finalTotal.toLocaleString('en-IN')})` : "Place Order (COD)")}
//                   </button>
...
//   const [userDetails, setUserDetails] = useState({
//       userId: '', // ⚡ Naya add kiya taaki coupon validation me kaam aaye
//       email: '',
//       firstName: '',
//       lastName: '',
//       countryCode: '+91', 
...
//             const profileRes = await axiosInstance.get('/auth/profile');
//             if (profileRes.data?.success && profileRes.data?.user) {
//                 // ⚡ userId bhi state me save kar rahe hain
//                 setUserDetails(prev => ({ 
//                     ...prev, 
//                     email: profileRes.data.user.email || prev.email,
...
//       try {
//           // ⚡ Yahan userId bhej rahe hain taaki backend First Order verify kar sake
...
//         try {
//             await axiosInstance.post('/auth/address', {
//                 firstName, lastName, phone: fullPhoneNumber,
//                 addressLine1: address, city, state, pincode: postalCode, country
//             });
//         } catch (err) {
//             console.warn("Failed to save address for future use", err);
//         }
...
//                 paymentMethod: 'cod',
//                 orderDetails: {
//                     email, items: formattedItems, totalAmount: finalTotal,
//                     shippingAddress: { fullName, phone: fullPhoneNumber, addressLine1: address, city, state, pincode: postalCode, country },
//                     couponApplied: isCouponApplied ? couponCode : null,
//                     discountAmount: discount
//                 }
//             });
...
import React, { useState, useEffect } from 'react';
import { Lock, ChevronDown, X, Tag, CheckCircle } from 'lucide-react';
import { useNavigate } from 'react-router-dom';
import axiosInstance from '../utils/axiosInstance';
import Toast from '../components/Toast';
