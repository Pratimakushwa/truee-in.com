...
export default function AdminOrders() {
  ...
  // ⚡ FIX: Modal logic update kiya
  const handleStatusChange = async (orderId, newStatus) => {
    const order = orders.find((o) => o._id === orderId);

    // Condition: Agar status 'Shipped' hai aur tracking details MISSING hain, tabhi modal dikhao
    if (newStatus === 'Shipped' && (!order.trackingDetails?.awbNumber)) {
      setShippingModal({ isOpen: true, orderId });
      return;
    }
    
    // Agar details hain, ya status kuch aur hai, toh direct update karo
    updateOrderStatus(orderId, newStatus);
  };
