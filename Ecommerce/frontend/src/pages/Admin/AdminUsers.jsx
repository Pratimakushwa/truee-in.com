...
import React, { useState, useEffect } from 'react';
import axiosInstance from '../../utils/axiosInstance'; // ⚡ ASLI IMPORT
import { useAuth } from '../../context/AuthContext';   // ⚡ ASLI IMPORT
...
export default function AdminUsers() {
  ...
  useEffect(() => {
    const fetchCustomers = async () => {
      try {
        // Ye line seedha tumhare backend database se hit karegi
        const response = await axiosInstance.get('/admin/users');
        setCustomers(response.data.users || []);
        setLoading(false);
      } catch (err) {
