...
// ⚡ ASLI IMPORTS (Mock data hata diya gaya hai)
import { useAuth } from '../../context/AuthContext';
...
export default function AdminDashboard() {
  ...
  // 1. Ek hi state mein teeno counts store karenge
  const [stats, setStats] = useState({ productCount: 0, orderCount: 0, userCount: 0 });

  // 2. Real API se data fetch karne ka logic
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        // ⚡ Ye tumhare backend ka actual route hit karega
