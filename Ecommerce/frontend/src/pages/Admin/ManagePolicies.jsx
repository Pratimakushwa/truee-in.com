...
import React, { useState, useEffect } from 'react';
import api from '../../utils/axiosInstance';
import { Save, Plus, Trash2, FileText, ShieldCheck, RefreshCcw, Info, LayoutTemplate } from 'lucide-react';
...
export default function ManagePolicies() {
  ...
  const handleSave = async (e) => {
    ...
    setIsSaving(true);
    try {
      await api.post('/legal/update-policy', { pageType, ...formData });
      alert(`Success! ${pageType.toUpperCase()} page has been updated.`);
    } catch (err) {
      console.error(err);
  ...
  // Premium UI Components Config (Updated to Light Theme)
  const inputStyle = "w-full bg-white border border-gray-200 p-4 rounded-xl text-sm text-gray-900 placeholder-gray-400 focus:border-[#C8A253] focus:outline-none focus:ring-1 focus:ring-[#C8A253] hover:border-gray-300 transition-all";
  ...
  return (
    <div className="w-full rounded-2xl bg-gray-50 min-h-screen text-gray-800 p-4 md:p-8 font-sans">
      ...
      {/* HEADER */}
      <div className="mb-10">
        <h1 className="text-4xl font-serif text-gray-900 mb-3">Legal & Policies</h1>
        <p className="text-gray-500 text-sm flex items-center gap-2">
          <Info size={16} className="text-[#C8A253]"/> 
          Manage content for the Truee Luxury storefront.
        </p>
      </div>
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        ...
        {/* LEFT COLUMN: TABS */}
        <div className="lg:w-1/4">
          <div className="sticky top-8 bg-white rounded-2xl p-2 border border-gray-200 flex flex-col gap-2 shadow-sm">
            {policyTabs.map((tab) => (
              <button
                key={tab.id}
                type="button"
                ...
                className={`flex items-center gap-3 w-full p-4 rounded-xl text-sm font-medium transition-all duration-300 ${
                  pageType === tab.id 
                    ? 'bg-[#C8A253]/10 border-l-2 border-[#C8A253] text-[#C8A253]' 
        ...
        <div className="lg:w-3/4">
          {loading ? (
            <div className="h-64 flex items-center justify-center bg-white rounded-2xl border border-gray-200 shadow-sm">
              <div className="w-8 h-8 border-2 border-gray-100 border-t-[#C8A253] rounded-full animate-spin"></div>
            </div>
          ) : (
            <form onSubmit={handleSave} className="space-y-6">
              
              {/* HEADER DETAILS BOX */}
              <div className="bg-white p-6 md:p-8 rounded-2xl border border-gray-200 shadow-sm transition-all duration-300">
                <h2 className="text-lg font-serif text-gray-900 mb-6 border-b border-gray-100 pb-4">Header Configuration</h2>
                
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                  <div>
                    <label className={labelStyle}>Page Title</label>
                    <input className={inputStyle} placeholder="e.g., Terms & Conditions" value={formData.heroTitle} onChange={(e) => setFormData({...formData, heroTitle: e.target.value})} required />
                  </div>
                  <div>
                    <label className={labelStyle}>Last Updated</label>
                    <input className={inputStyle} placeholder="e.g., July 2026" value={formData.lastUpdated} onChange={(e) => setFormData({...formData, lastUpdated: e.target.value})} />
                  </div>
                </div>

                <div>
                  <label className={labelStyle}>Subtitle / Tagline</label>
                  <input className={inputStyle} placeholder="Add a short tagline" value={formData.subtitle} onChange={(e) => setFormData({...formData, subtitle: e.target.value})} />
                </div>
              </div>

              {/* CONTENT SECTIONS BOX */}
              <div className="bg-white p-6 md:p-8 rounded-2xl border border-gray-200 shadow-sm transition-all duration-300">
                <h2 className="text-lg font-serif text-gray-900 mb-6 border-b border-gray-100 pb-4">Content Sections</h2>
                
                <div className="space-y-6">
                  {formData.sections.map((sec, i) => (
                    <div key={i} className="relative bg-gray-50 p-6 rounded-xl border border-gray-100 group hover:border-gray-200 transition-all">
                      
                      {formData.sections.length > 1 && (
                        <button type="button" onClick={() => removeSection(i)} className="absolute -top-3 -right-3 bg-white text-red-500 p-2 rounded-full border border-gray-200 hover:bg-red-50 transition-all z-10 shadow-sm" title="Delete">
                          <Trash2 size={16} />
                        </button>
                      )}
                      ...
                      <div className="space-y-4">
                        <div>
                          <label className="block text-[10px] font-bold text-gray-500 mb-2">SECTION {i + 1} TITLE</label>
                          <input className={inputStyle} placeholder="Enter heading" value={sec.title} required onChange={(e) => { const newS = [...formData.sections]; newS[i].title = e.target.value; setFormData({...formData, sections: newS}); }} />
                        </div>
                        <div>
                          <label className="block text-[10px] font-bold text-gray-500 mb-2">DESCRIPTION</label>
                          <textarea className={`${inputStyle} min-h-[120px]`} placeholder="Write policy details..." value={sec.text} required onChange={(e) => { const newS = [...formData.sections]; newS[i].text = e.target.value; setFormData({...formData, sections: newS}); }} />
                        </div>
                      </div>
                    </div>
                  ))}
                ...
                <button 
                  type="button" 
                  onClick={() => setFormData({...formData, sections: [...formData.sections, { title: '', text: '' }]})} 
                  className="mt-8 flex items-center justify-center gap-2 w-full border border-dashed border-gray-300 text-sm font-bold text-gray-500 bg-gray-50 p-4 rounded-xl hover:border-[#C8A253] hover:text-[#C8A253] transition-all"
                >
                  <Plus size={16} strokeWidth={3} /> Add New Section
                </button>
              </div>
              ...
              <div className="flex justify-end pt-4">
                <button 
                  type="submit" 
                  disabled={isSaving}
                  className="flex items-center gap-3 bg-gray-900 text-white px-10 py-4 rounded-xl font-bold uppercase tracking-widest text-[11px] hover:bg-gray-800 hover:shadow-lg transition-all duration-300 disabled:opacity-50"
                >
                  {isSaving ? 'Publishing...' : 'Publish to Store'} <Save size={16} />
                </button>
              </div>
