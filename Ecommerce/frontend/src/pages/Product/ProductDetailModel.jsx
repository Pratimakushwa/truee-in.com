...
//           {/* ⚡ Yahan Image box ko aur compact kar diya gaya hai bina image kharab kiye */}
//           <div className="w-full lg:w-[45%] lg:sticky lg:top-24 self-start">
//             <div className="bg-[#f7f7f7] w-full max-w-[480px] aspect-square rounded-[2rem] flex items-center justify-center p-6 relative overflow-hidden group mx-auto cursor-pointer">
//               <img src={galleryImages[activeImgIdx]} className="w-full h-full object-contain mix-blend-multiply transition-transform duration-700 ease-in-out group-hover:scale-110" alt="main" style={{ maxHeight: '380px', marginTop: '-10px' }} />
//             </div>
...
//                   key={i} 
//                   onClick={() => setActiveImgIdx(i)} 
//                   className={`w-[80px] h-[110px] md:w-[100px] md:h-[130px] rounded-[1.5rem] bg-[#f7f7f7] p-3 border-[2px] cursor-pointer transition-all duration-300 ${
//                     activeImgIdx === i ? 'border-black shadow-sm' : 'border-transparent hover:border-gray-200'
//                   }`}
//                 >
//                   <img src={img} className="w-full h-full object-contain mix-blend-multiply" alt="thumb" />
...
//         {/* ⚡ Dynamic Video Code */}
//         {fullProduct?.promotionalVideo?.videoUrl && (
//           <section id="video" className="mb-16 px-6 md:px-12">
//             <div className="w-full h-64 md:h-[500px] bg-black rounded-[3rem] overflow-hidden shadow-lg relative">
//               <video 
...
//               >
//                 <source src={fullProduct.promotionalVideo.videoUrl} />
//                 Your browser does not support the video tag.
//               </video>
//             </div>
//           </section>
...
//   AudioLines, Speaker, Headphones, Volume2 
// } from 'lucide-react';
// import axiosInstance from '../../utils/axiosInstance';

// const DEFAULT_IMG = "https://images.unsplash.com/photo-1523275335684-37898b6baf30?auto=format&fit=crop&q=80&w=600";

// // ⚡ 1. Naye Icons aur Aliases add kiye hain
// const iconMap = { 
//   Battery: <Battery size={24} />,
//   Droplets: <Droplets size={24} />,
//   Wifi: <Wifi size={24} />,
...
//   Setting: <Settings size={24} />, // Alias
//   Audio: <AudioLines size={24} />,
//   AudioLines: <AudioLines size={24} />,
//   Speaker: <Speaker size={24} />,
//   Headphones: <Headphones size={24} />,
//   Earbuds: <Headphones size={24} />, // Alias
//   Vol: <Volume2 size={24} />
// };

// // ⚡ 2. Smart Function: Capital/Small letter ka issue theek karne ke liye
// const getIcon = (iconName) => {
//   if (!iconName) return <Tag size={24} />;
//   const searchKey = String(iconName).trim().toLowerCase();
//   const match = Object.keys(iconMap).find(k => k.toLowerCase() === searchKey);
//   return match ? iconMap[match] : <Tag size={24} />;
// };

// // ⚡ 3. YEH FUNCTION WAPAS DAAL DIYA HAI (Jis se Error aa raha tha)
...
//         {/* ⚡ SMART DYNAMIC VIDEO SECTION (Supports YouTube & MP4) */}
//       {/* ⚡ VIDEO SECTION */}
//          {/* ⚡ VIDEO SECTION */}
//           {(fullProduct?.promotionalVideo?.videoUrl || fullProduct?.promotionalVideo?.url) && (
//             <section id="video" className="max-w-[1340px] mx-auto px-6 md:px-12 mb-20">
//               <div className="w-full h-[300px] md:h-[450px] bg-black rounded-[2rem] md:rounded-[3rem] overflow-hidden shadow-2xl relative pointer-events-none">
//                 {(() => {
...
//                     return (
//                       <iframe
//                         // ⚡ ABSOLUTE CENTER TRICK: Iframe ko hamesha 16:9 rakha aur container se bada kar diya taaki black bars bahar nikal jayein!
//                         className="absolute top-1/2 left-1/2 w-[250%] md:w-[150%] aspect-video max-w-none -translate-x-1/2 -translate-y-1/2"
...
//   {fullProduct?.brand ? `${fullProduct.brand} Signature Sound.` : "Premium Signature Sound."}
// </h2>            <p className="text-[11px] font-bold uppercase tracking-[0.25em] text-gray-500 mb-16">{fullProduct?.reviewsCount || "843"} Verified Audiophiles &nbsp;•&nbsp; 4.9 Rating</p>
...
//   Vol: <Volume2 size={24} />,
//   Star: <Star size={24} />,
//   Home: <Home size={24} />,   // ⚡ Yahan add kiya
//   Award: <Award size={24} />  // ⚡ Yahan add kiya
// };

// // ⚡ 2. Smart Function for Icons
// const getIcon = (iconName) => {
//   if (!iconName) return <Tag size={24} />;
//   const searchKey = String(iconName).trim().toLowerCase();
//   const match = Object.keys(iconMap).find(k => k.toLowerCase() === searchKey);
...
//                     {(fullProduct?.highlights || []).map((h, i) => (
//                       <div key={i} className="flex flex-col items-center text-center gap-2 group">
//                         {/* ⚡ YAHAN YEH FIX KIYA HAI: getIcon() function ka use kiya hai */}
...
//             <div className="w-full h-[300px] md:h-[500px] bg-black rounded-[2rem] md:rounded-[3rem] overflow-hidden shadow-2xl relative pointer-events-none">
//               {(() => {
//                 const videoUrl = fullProduct.promotionalVideo.videoUrl || fullProduct.promotionalVideo.url;
//                 const ytId = getYouTubeId(videoUrl);
...
//   Shield, Tv, Volume, Smartphone, Mic, Plus, Tag, Music, Sun, Settings, 
//   AudioLines, Speaker, Headphones, Volume2, Home, Award,
//   Briefcase, BatteryCharging, Crown,EarOff, PhoneCall, Sliders, Feather // ⚡ Naye icons import kiye
// } from 'lucide-react';
// import axiosInstance from '../../utils/axiosInstance';

// const DEFAULT_IMG = "https://images.unsplash.com/photo-1523275335684-37898b6baf30?auto=format&fit=crop&q=80&w=600";

// // ⚡ 1. Icons Mapping (Tumhare Admin Panel wale exact words add kar diye)
// const iconMap = { 
//   Battery: <Battery size={24} />,
//   Droplets: <Droplets size={24} />,
...
//   // 🔥 TUMHARE ADMIN PANEL WALE EXACT WORDS:
...
//   // Agar naam match nahi hua toh title padh kar icon dega
...
//                       <div key={i} className="flex flex-col items-center text-center gap-2 group">
//                         <div className="text-gray-800 opacity-70 group-hover:scale-110 transition-transform">
//                           {/* ⚡ SMART ICON GETTER USE KIYA HAI */}
