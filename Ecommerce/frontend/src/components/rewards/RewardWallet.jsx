    ...
    <div className="p-5 sm:p-6 md:p-8">
      <div className="mb-6 md:mb-8">
        <h2 className="text-xl md:text-2xl font-serif font-bold text-gray-900 flex items-center gap-2">
          <Wallet className="text-[#C8A253]" size={22} /> Reward Wallet
        </h2>
        <p className="text-xs sm:text-sm text-gray-500 mt-1">{settings?.rewardMessage}</p>
      </div>

      {/* Wallet Cards */}
      <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 mb-6 md:mb-8">
        <div className="col-span-2 lg:col-span-1 bg-gradient-to-br from-[#FCFAEF] to-[#F5EEDA] border border-[#C8A253]/30 text-gray-900 rounded-2xl p-5 sm:p-6 shadow-sm">
          <p className="text-[9px] sm:text-[10px] uppercase tracking-widest text-[#8B6914] font-bold mb-2">Available Coins</p>
          <p className="text-3xl sm:text-4xl font-bold text-[#111]">{w.availableCoins ?? 0}</p>
          <p className="text-xs sm:text-sm text-gray-500 mt-1">Wallet Value ₹{w.walletValue ?? 0}</p>
        </div>
        <div className="bg-white border border-gray-200 rounded-2xl p-4 sm:p-5">
          <TrendingUp size={16} className="text-green-600 mb-2" />
          <p className="text-[9px] sm:text-[10px] uppercase tracking-widest text-gray-400">Lifetime Earned</p>
          <p className="text-xl sm:text-2xl font-bold text-gray-900">{w.lifetimeEarned ?? 0}</p>
        </div>
        <div className="bg-white border border-gray-200 rounded-2xl p-4 sm:p-5">
          <Gift size={16} className="text-[#C8A253] mb-2" />
          <p className="text-[9px] sm:text-[10px] uppercase tracking-widest text-gray-400">Lifetime Redeemed</p>
          <p className="text-xl sm:text-2xl font-bold text-gray-900">{w.lifetimeRedeemed ?? 0}</p>
        </div>
        <div className="bg-white border border-gray-200 rounded-2xl p-4 sm:p-5">
          <Clock size={16} className="text-gray-400 mb-2" />
          <p className="text-[9px] sm:text-[10px] uppercase tracking-widest text-gray-400">Pending / Expired</p>
          <p className="text-base sm:text-lg font-bold text-gray-900">{w.pendingCoins ?? 0} / {w.expiredCoins ?? 0}</p>
        </div>
      </div>
      ...
      <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden">
        <div className="p-4 md:p-6 border-b border-gray-100 flex flex-col md:flex-row md:items-center justify-between gap-3 md:gap-4">
          <h3 className="font-bold text-gray-900 flex items-center gap-2 text-sm sm:text-base">
            <Coins size={17} className="text-[#C8A253]" /> Transaction History
          </h3>
          <div className="flex flex-wrap gap-2">
            <div className="relative flex-1 min-w-[140px] sm:min-w-0">
              ...
              <input
                ...
                value={search}
                onChange={(e) => setSearch(e.target.value)}
                className="pl-8 pr-3 py-2 border border-gray-200 rounded-lg text-xs sm:text-sm w-full sm:w-40"
              />
            </div>
            <select
              value={typeFilter}
              onChange={(e) => setTypeFilter(e.target.value)}
              className="border border-gray-200 rounded-lg px-3 py-2 text-xs sm:text-sm flex-1 sm:flex-none min-w-[110px]"
            >
              <option value="all">All Types</option>
              {Object.entries(TYPE_LABELS).map(([k, v]) => (
                <option key={k} value={k}>{v}</option>
            ...
            <button onClick={handleExport} className="flex items-center gap-1 px-3 py-2 border border-gray-200 rounded-lg text-xs sm:text-sm hover:bg-gray-50 shrink-0">
