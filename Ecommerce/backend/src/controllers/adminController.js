...
// ⚡ DASHBOARD STATS LOGIC (Sirf ek baar aur ekdum sahi) ⚡
module.exports.getDashboardStats = wrapAsync(async (req, res) => {
  const productCount = await Product.countDocuments();
  const orderCount = await Order.countDocuments();
  const userCount = await User.countDocuments({ role: 'customer' });
  const couponCount = await Coupon.countDocuments(); // Naya Coupon count
  
  res.status(200).json({
    success: true,
    stats: { productCount, orderCount, userCount, couponCount } 
  });
});