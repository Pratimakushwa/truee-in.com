...
// ⚡ Bloom Filter Import
const { couponFilter } = require('../utils/bloomFilter');
...
exports.createCoupon = async (req, res) => {
    try {
        ...
        // ⚡ Naya coupon create hote hi Bloom Filter mein add kar do
        if (newCoupon.code && newCoupon.status === 'Active') {
            couponFilter.add(newCoupon.code.trim().toUpperCase());
        }
...
exports.verifyCoupon = async (req, res) => {
    try {
        ...
        // ⚡ 1. BLOOM FILTER CHECK: Agar code filter mein hai hi nahi, toh DB hit karne ki zaroorat hi nahi!
        if (!couponFilter.has(formattedCode)) {
            return res.status(404).json({ message: "Coupon code invalid!" });
        }

        // ⚡ 2. Agar filter bole '1' (Probably present), tabhi MongoDB check karo
...
exports.updateCouponStatus = async (req, res) => {
    try {
        ...
        // Agar status Active ho gaya hai, toh filter me add kar do
        if (status === 'Active' && updatedCoupon.code) {
            couponFilter.add(updatedCoupon.code.trim().toUpperCase());
        }
