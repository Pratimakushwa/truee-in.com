...
//         images: [], 
//         variants: [],
//         lifestyleImages: [] // ⚡ Initialize lifestyle array
//     };
...
//     // ⚡ NEW: LIFESTYLE IMAGES UPLOAD LOGIC
//     const lifestyleFiles = req.files?.filter(f => f.fieldname === 'lifestyleImages') || [];
//     for (const file of lifestyleFiles) {
//         const optimizedBuffer = await sharp(file.buffer).webp({ quality: 80 }).toBuffer();
//         const key = `Products/Lifestyle/${Date.now()}-${Math.random().toString(36).substring(2, 7)}.webp`;
        
//         await r2.send(new PutObjectCommand({
//             Bucket: process.env.R2_BUCKET_NAME, Key: key, Body: optimizedBuffer, ContentType: 'image/webp',
//         }));
//         productData.lifestyleImages.push({ public_id: key, url: `${process.env.R2_PUBLIC_URL}/${key}` });
//     }
...
//     // ⚡ NEW: LIFESTYLE IMAGES UPDATE LOGIC
//     let updatedLifestyleImages = [];
//     if (req.body.existingLifestyleImages) {
//         updatedLifestyleImages = typeof req.body.existingLifestyleImages === 'string' 
//             ? JSON.parse(req.body.existingLifestyleImages) 
//             : req.body.existingLifestyleImages;
//     }
...
//             Bucket: process.env.R2_BUCKET_NAME, Key: key, Body: optimizedBuffer, ContentType: 'image/webp',
//         }));
//         updatedLifestyleImages.push({ public_id: key, url: `${process.env.R2_PUBLIC_URL}/${key}` });
//     }
//     req.body.lifestyleImages = updatedLifestyleImages;
...
exports.createProduct = wrapAsync(async (req, res, next) => {
    ...
    // ⚡ BULLETPROOF VIDEO SAVE LOGIC (UNTOUCHED)
    if (req.body.promotionalVideo) {
        const parsedVideo = typeof req.body.promotionalVideo === 'string' ? JSON.parse(req.body.promotionalVideo) : req.body.promotionalVideo;
        productData.promotionalVideo = {
            videoUrl: parsedVideo.videoUrl || '',
    ...
    // ⚡ SMART LIFESTYLE IMAGES (WITH TITLE & DESCRIPTION SUPPORT)
    let lifestyleMeta = [];
    if (req.body.lifestyleMetadata) {
        try {
            lifestyleMeta = typeof req.body.lifestyleMetadata === 'string' 
                ? JSON.parse(req.body.lifestyleMetadata) 
                : req.body.lifestyleMetadata;
        } catch (e) {
            lifestyleMeta = [];
        }
    }
    ...
    const lifestyleFiles = req.files?.filter(f => f.fieldname === 'lifestyleImages') || [];
    let fileIndex = 0;
    let processedLifestyle = [];

    for (let i = 0; i < lifestyleMeta.length; i++) {
        const meta = lifestyleMeta[i];

        if (meta.type === 'existing' && meta.existingData) {
            processedLifestyle.push({
                image: meta.existingData.image || meta.existingData,
                title: meta.title || '',
                description: meta.description || ''
            });
        } else if (meta.type === 'new' && lifestyleFiles[fileIndex]) {
            const file = lifestyleFiles[fileIndex];
            fileIndex++;
            ...
            processedLifestyle.push({
                image: { public_id: key, url: `${process.env.R2_PUBLIC_URL}/${key}` },
                title: meta.title || '',
                description: meta.description || ''
            });
        }
    }
    productData.lifestyleImages = processedLifestyle;
...
// -------------------------------------------------------------------
// 3. GET SINGLE PRODUCT (UNTOUCHED)
// -------------------------------------------------------------------
exports.getProductDetails = wrapAsync(async (req, res, next) => {
    const { id } = req.params;
    if (!mongoose.Types.ObjectId.isValid(id)) throw new ExpressError(400, 'Invalid ID');
...
exports.updateProduct = wrapAsync(async (req, res, next) => {
    ...
    // ⚡ BULLETPROOF VIDEO UPDATE LOGIC (UNTOUCHED)
    if (req.body.promotionalVideo) {
        const parsedVideo = typeof req.body.promotionalVideo === 'string' ? JSON.parse(req.body.promotionalVideo) : req.body.promotionalVideo;
        req.body.promotionalVideo = {
            videoUrl: parsedVideo.videoUrl || '',
    ...
    // ⚡ SMART LIFESTYLE UPDATE LOGIC (WITH TITLE & DESCRIPTION)
    let lifestyleMeta = [];
    if (req.body.lifestyleMetadata) {
        try {
            lifestyleMeta = typeof req.body.lifestyleMetadata === 'string' 
...
// -------------------------------------------------------------------
// 6. GET ADMIN PRODUCTS (UNTOUCHED)
// -------------------------------------------------------------------
exports.getAdminProducts = async (req, res, next) => {
    try {
        const products = await Product.find();
...
// -------------------------------------------------------------------
// 7. TOGGLE FEATURED STATUS (UNTOUCHED)
// -------------------------------------------------------------------
exports.toggleFeaturedStatus = wrapAsync(async (req, res, next) => {
    const product = await Product.findById(req.params.id);
    if (!product) return next(new ExpressError(404, 'Product not found'));
